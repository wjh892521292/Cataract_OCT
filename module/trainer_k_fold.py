...
from utils.loss import RMSELoss, CVALoss
...
class DefaultTrainer(object):
    ...
    def __init__(self, args, k_fold=""):
        ...
        self.model = getattr(models_new, args.model_name.lower())(args)
        self.model.cuda()
        self.SMAE = nn.SmoothL1Loss()
        self.MAE = nn.L1Loss()
        self.MSE = nn.MSELoss()
        self.RMSE = RMSELoss()
    ...
    def train_iter(self, step, dataloader):
        ...
        target3 = (target2 >= 0.2 - eps).long().float().cuda()
        ...
        loss1 = self.RMSE(score, target)
        loss2 = loss3 = self.MAE(score_d, target2)


        if self.args.cls_true:
    ...
    def validation(self, step, val_iter, val_epoch_size):
        ...
        loss_reg = self.RMSE(total_score, total_target)
        loss_reg_d = loss_cls = self.MAE(total_score2, total_target2)
        ...
        return acc, loss_reg_d, loss_reg
    ...
    def _adjust_learning_rate_iter(self, step):
        ...
        if self.args.lr_adjust == 'fix':
            ...
            if step > self.warmup_steps:
                self.lr_current = self.args.lr * (1 - (step - self.warmup_steps) / self.args.max_iter) ** 0.9

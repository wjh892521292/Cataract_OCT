...
class DefaultTrainer(object):
    ...
    def train_iter(self, step, dataloader):
        ...
        elif self.args.model_name.lower() == 'resnet2' or self.args.model_name.lower() == 'resnet3' or self.args.model_name.lower() == 'resnet4' or self.args.model_name.lower() == 'cbt1':
        ...
        with torch.no_grad():
            for i in range(val_epoch_size):
                ...
                if i == 0:
                    ...
                    total_score2 = torch.cat((total_score2, score), 0)

                    total_target = torch.cat((total_target, target), 0)

        loss = self.MAE(total_score, total_target)
        loss2 = self.MAE(total_score2, total_target)

        pred = (total_score >= 0.2 ).cpu()
        gt = (total_target >= 0.2).cpu()

...
class CrossTransformer(nn.Module):
    ...
    # make some changes to the end layer contrast to the original resnet
    def forward(self, x, z, zz):
        p = torch.cat((x, zz, z), 1)
        p = self.transformer_encoder(p)
        x = p[:, 0, :].contiguous().unsqueeze(1)
        z = p[:, -1, :].contiguous().unsqueeze(1)
        zz = p[:, 1, :].contiguous().unsqueeze(1)
...
class Ctt(nn.Module):
    ...
    def forward(self, x, y, z1, z2, z):
        ...
        x, z1, zz1 = self.Transformer_1(x, z1, z)
        y, z2, zz2 = self.Transformer_2(y, z2, z)
        ...
        for layer in range(self.layers):
            ...
            x, z1, zz1 = self.Transformer_x(x, z1, zz1)
            y, z2, zz2 = self.Transformer_y(y, z2, zz2)
